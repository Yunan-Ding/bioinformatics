...
# 读取数据
df = pd.read_excel('E:\Python training\AREG\hTFDB-TRANSFAC+JASPAR.xlsx',engine='openpyxl')
#print(df)

# 按照转录因子名字出现次数进行计数
name = df.groupby('TF').size()
name_num = df.groupby('TF').count()
# print(name_num)
...
nx.draw(G, with_labels=True,edge_color='grey',node_color='lavender',font_size =4,
        nodelist=dd, node_size=[v ** 1.5 for v in dd.values()],
        width=[float(d['weight'] * 0.1) for (u, v, d) in G.edges(data=True)])

plt.show()

result3.to_csv('E:\Python training\AREG\hTFDB-Trans-Jas.csv',index=False)